...
def build_pybind(ctx, modules, fd):
    ...
    for module in modules:
        ...
        if local_module_prefix is None:
            local_module_prefix = module.search_one(
                'belongs-to').search_one('prefix')
            if local_module_prefix is None:
...
def build_typedefs(ctx, defnd):
    ...
    for i_tuple in process_typedefs_ordered:
        ...
        cls, elemtype = copy.deepcopy(
            build_elemtype(
                ctx, item.search_one('type')))
        known_types = class_map.keys()
        ...
        if not isinstance(elemtype, list):
            ...
            if "referenced_path" in elemtype:
                class_map[type_name]["referenced_path"] = elemtype[
                    "referenced_path"]
                class_map[type_name]["class_override"] = "leafref"
            if "require_instance" in elemtype:
                class_map[type_name]["require_instance"] = elemtype[
                    "require_instance"]
    ...
    if len(elements) == 0:
        ...
        for i in elements:
            ...
            if i["class"] == "leaf-list":
                ...
                # TypedList (see lib.yangtypes) with a particular set of types
                # allowed.
                ...
                class_str[
                    "arg"] += ", yang_name=\"%s\", parent=self" % (i["yang_name"])
                ...
                class_str[
                    "arg"] += "(referenced_path='%s'" % i["referenced_path"]
                class_str["arg"] += ", caller=self._path() + ['%s'], " \
                    % (i["yang_name"])
                class_str["arg"] += "path_helper=self._path_helper, "
                class_str[
                    "arg"] += "require_instance=%s)" % (i["require_instance"])
                ...
                class_str[
                    "arg"] += "caller=self._path() + ['%s'], " % i["yang_name"]
                class_str["arg"] += "path_helper=self._path_helper, "
                class_str["arg"] += "require_instance=%s))" % \
                    (i["type"]["native_type"][1]["require_instance"])
            else:
                # Generically handle all other classes with the 'standard'
                # mappings.
                class_str["name"] = "__%s" % (i["name"])
                class_str["type"] = "YANGDynClass"
  ...
  def __init__(self, *args, **kwargs):\n""")
    ...
    if args:
      ...
      for e in self._pyangbind_elements:
        ...
        # a path in the form of a list that describes the nodes in the
        # hierarchy.
        nfd.write("""
  def _path(self):
    ...
    if length_stmt:
        if "|" in length_stmt.arg:
            restrictions['length'] = [
                i.replace(
                    ' ',
                    '') for i in length_stmt.arg.split("|")]
        else:
    ...
    if cls is None:
        ...
        # has been specified.
            ...
            if base_stmt is None:
                raise ValueError(
                    "identityref specified with no base statement")
            try:
...
def get_element(ctx, fd, element, module, parent, path,
    ...
    if hasattr(element, 'i_children'):
        ...
        if element.i_children:
            ...
            # were asked to split the bindings into a directory structure or
            # not.
            if ctx.opts.split_class_dir:
                ...
                # Otherwise, give a unique name for the class within the
                # dictionary.
                    ...
                    if check not in checked:
                        ...
                        if "parent_type" in tmp_class_map[check]:
                            if isinstance(tmp_class_map[check]["parent_type"], list):
                                to_visit.extend(
                                    tmp_class_map[check]["parent_type"])
                            else:
                                to_visit.append(
                                    tmp_class_map[check]["parent_type"])
