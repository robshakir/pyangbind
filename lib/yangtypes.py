    ...
    class RestrictedPrecisionDecimal(Decimal):
        ...
        def __new__(self, *args, **kwargs):
            ...
            if not self._precision is None:
                if len(args):
                    value = Decimal(args[0]).quantize(
                        Decimal(str(self._precision)))
                else:
                    value = Decimal(0)
            elif len(args):
...
def RestrictedClassType(*args, **kwargs):
    ...
    # class
    current_restricted_class_type = re.sub(
        "<(type|class) '(?P<class>.*)'>",
        "\g<class>",
        str(base_type))
      ...
      """
            ...
            def build_length_range_tuples(range, length=False):
                if range_regex.match(range_spec):
                    low, high = range_regex.sub(
                        "\g<low>,\g<high>", range_spec).split(",")
                    if not length:
                    ...
                    eqval = range_single_value_regex.sub(
                        '\g<value>',
                        range_spec)
                    if not length:
                        eqval = base_type(eqval) if not eqval in [
                            "max",
                            "min"] else None
                    else:
                        eqval = int(eqval)
                    ...
                    raise ValueError(
                        "Invalid range or length argument specified")
                ...
                for check_tuple in low_high_tuples:
                    ...
                    if all_none:
                        raise AttributeError(
                            "Cannot specify a range that is all max and min")
                ...
                def range_check(value):
                    ...
                    for check_tuple in low_high_tuples:
                        ...
                        if len(check_tuple) == 2:
                            ...
                            raise AttributeError(
                                "Invalid check tuple length specified")
            ...
            if self._restriction_dict is None:
                if restriction_type is not None and restriction_arg is not None:
                    self._restriction_dict = {
                        restriction_type: restriction_arg}
                else:
                    raise ValueError(
                        "must specify either a restriction dictionary or a type and argument")
                    ...
                    if val:
                        try:
                            ...
                            if not float(val) == float(preval):
                                raise ValueError(
                                    "Could not set value, overflowed when cast to native type")
                        except:
                            raise TypeError(
                                "must specify a numeric type for a range argument")
                elif rtype == "length":
                    lengths = []
                    for range_spec in rarg:
                        lengths.append(
                            build_length_range_tuples(
                                range_spec,
                                length=True))
                    self._restriction_tests.append(
                        in_range_check(
                            lengths,
                            length=True))
                elif rtype == "dict_key":
            ...
            if not val == False:
                ...
                if not passed:
                    raise ValueError(
                        "%s does not match a restricted type" %
                        val)
...
def YANGListType(*args, **kwargs):
    ...
    try:
        ...
        raise TypeError(
            "A YANGList must be specified with a key value and a contained class")
    ...
    class YANGList(object):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if not isinstance(listclass, type(int)):
                raise ValueError(
                    "contained class of a YANGList must be a class")
                ...
                try:
                    ...
                    if " " in self._keyval:
                        ...
                        if not len(keyparts) == len(keys):
                            raise KeyError(
                                "YANGList key must contain all key elements (%s)" %
                                (self._keyval.split(" ")))
                        path_keystring = "["
                        for kv, kp in zip(keys, keyparts):
                            kv_obj = getattr(tmp, kv)
                            path_keystring += "%s='%s' " % (kv_obj.yang_name(),
                                                            kp)
                        ...
                        if k == "":
                            raise KeyError(
                                "Cannot set a null key for a list entry!")
                        keys = [self._keyval, ]
                        keyparts = [k, ]
                    ...
                    if not update:
                        ...
                        # hand the value to the init, rather than simply
        ...
        def add(self, k=None):
            ...
            if self._keyval:
                if k is None:
                    raise KeyError(
                        "a list with a key value must have a key specified")
                self.__set(k)
            else:
                k = self.__set()
                return k
        ...
        def delete(self, k):
            if self._path_helper:
                ...
                obj_path = self._parent._path(
                ) + [self._yang_name + key_string]
    ...
    def __new__(self, *args, **kwargs):
        ...
        if len(args):
            if not args[0] in false_args + true_args:
                raise ValueError(
                    "%s is an invalid value for a YANGBool" %
                    args[0])
    ...
    if isinstance(base_type, list):
        ...
        if not len(args):
            ...
            for candidate_type in base_type:
                try:
                    type_test = candidate_type(
                        args[0])  # does the slipper fit?
                    break
                except:
                    # don't worry, move on, plenty more fish (types) in the
                    # sea...
                    pass
            if not type_test:
    ...
    class YANGBaseClass(base_type):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if not self._is_container == "list":
                if self._path_helper:
                    if self._supplied_register_path is None:
                        ...
                        self._path_helper.register(
                            self._supplied_register_path,
                            self)
            ...
            try:
                ...
                raise TypeError(
                    "couldn't generate dynamic type -> %s -> %s" %
                    (type(e), e))
        ...
        def append(self, *args, **kwargs):
            if not hasattr(super(YANGBaseClass, self), "append"):
                raise AttributeError(
                    "%s object has no attribute append" %
                    base_type)
            self._set()
        ...
        def remove(self, *args, **kwargs):
            if not hasattr(super(YANGBaseClass, self), "remove"):
                raise AttributeError(
                    "%s object has no attribute remove" %
                    base_type)
            self._set()
            if self._path_helper:
        ...
        def extend(self, *args, **kwargs):
            if not hasattr(super(YANGBaseClass, self), "extend"):
                raise AttributeError(
                    "%s object has no attribute extend" %
                    base_type)
            self._set()
        ...
        def insert(self, *args, **kwargs):
            if not hasattr(super(YANGBaseClass, self), "insert"):
                raise AttributeError(
                    "%s object has no attribute insert" %
                    base_type)
            self._set()
    ...
    class ReferencePathType(object):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if self._path_helper and value is not None:
                path_chk = self._path_helper.get(
                    self._referenced_path,
                    caller=self._caller)
                ...
                if len(path_chk) == 1 and not path_chk[0]._is_keyval and not is_yang_list(path_chk[0]):
                    ...
                    if value:
                        set_method(value)
                    self._type = re.sub(
                        "<(type|class) '(?P<class>.*)'>", "\g<class>", str(get_method()._base_type))
                    self._ptr = True
                        ...
                        if len(path_chk) == 1 and is_yang_leaflist(path_chk[0]):
                            ...
                            for i in path_chk[0]:
                                if unicode(i) == unicode(value):
                                    found = True
                                    self._referenced_object = path_chk[
                                        0][index]
                                    break
                                index += 1
                        else:
