        ...
        def jsonmap(obj, map_val):
            ...
            if map_val in ["numpy.uint8", "numpy.uint16", "numpy.uint32", "numpy.uint64", "numpy.int8", "numpy.int16", "numpy.int32", "numpy.int64"]:
                ...
                raise AttributeError(
                    "this was an unmapped pyangbind class... (%s)" %
                    map_val)
...
class pybindJSONDecoder(object):
    ...
    def load_json(self, d, parent, yang_base, obj=None,
                  path_helper=None, extmethods=None, overwrite=False):
        if obj is None:
            # we need to find the class to create, as one has not been
            # supplied.
        ...
        for key in d:
            ...
            if child is None:
                raise AttributeError(
                    "JSON object contained a key that did not exist (%s)" %
                    (key))
            chobj = child()
            ...
            if pybind_attr in ["container"]:
                ...
                raise pybindJSONUpdateError(
                    "unknown pybind type when loading JSON: %s" %
                    pybind_attr)
