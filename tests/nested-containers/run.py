...
class NestedContainerTests(PyangBindTestCase):
    ...
    def test_subcontainer_is_not_changed_by_default(self):
        self.assertFalse(
            self.nested_obj.container.subcontainer._changed(),
            "subcontainer was marked to changed",
        )

    def test_container_is_not_changed_by_default(self):
        self.assertFalse(
            self.nested_obj.container._changed(), "container was marked to changed"
        )
    ...
    def test_subcontainer_get(self):
        self.nested_obj.container.subcontainer.a_leaf = 1
        self.assertEqual(
            self.nested_obj.container.subcontainer.get(),
            {"a-leaf": 1},
    ...
    def test_container_get(self):
        self.nested_obj.container.subcontainer.a_leaf = 1
        self.assertEqual(
            self.nested_obj.container.get(),
            {"subcontainer": {"a-leaf": 1}},
