...
def main():
    ...
    os.system(
        "%s --plugindir %s -f pybind -o %s/bindings.py %s/%s.yang" %
    ...
    for i in ["u1", "u2", "u3", "u4", "u6", "u7", "u8"]:
        assert hasattr(
            u.container, i), "container was missing attribute %s" % i
    ...
    assert isinstance(u.container.u4._default, numpy.int8), \
        "union with integer default did not have integer type default (%s)" % \
        type(u.container.u4._default)

    assert isinstance(u.container.u6._default, unicode), \
    ...
    for i in [(15, True), (35, True), ("aardvark", True), ("bear", True),
        ...
        assert passed == i[1], \
            ...
            + \
            "correctly validate the value (%s -> %s != %s)" % (
                i[0],
                passed,
                i[1])

    assert isinstance(u.container.u11._default, unicode), \
