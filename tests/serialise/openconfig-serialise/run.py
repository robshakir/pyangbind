...
class OpenconfigSerialiseTests(PyangBindTestCase):
    ...
    yang_files = [
        os.path.join(
            os.path.dirname(__file__), "openconfig", "openconfig-interfaces.yang"
        )
    ]
    pyang_flags = [
        "--use-xpathhelper",
    ...
    def test_json_generation(self):
        ...
        for file_name in os.listdir(json_dir):
            with self.subTest(json_file=file_name), open(
                os.path.join(json_dir, file_name), "r"
            ) as file_handle:
                ...
                path_helper, config_filter, mode = (
                    YANGBool(parameters[0]),
                ...
                self.assertEqual(
                    expected_json,
                    actual_json,
                    "Generated JSON did not match expected object for %s" % file_name,
                )
    ...
    def test_pybind_ietf_json_encoder_serialisation_with_path_helper(self):
        ...
        try:
            json.loads(
                json.dumps(
                    pybindIETFJSONEncoder.generate_element(instance),
        ...
        self.assertTrue(
            passed,
            "Serialisation test for object with pybindIETFJSONEncoder threw an error",
        )
