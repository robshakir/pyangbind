...
class IntTests(PyangBindTestCase):
    ...
    for name in ["eight", "sixteen", "thirtytwo", "sixtyfour"]:
      for subname in ["", "default", "result", "restricted"]:
        with self.subTest(name=name, subname=subname):
          self.assertTrue(hasattr(self.int_obj.int_container, "%s%s" % (name, subname)),
            "missing %s%s from container" % (name, subname))

  def test_default_values(self):
      for pair in [("eightdefault", 2 ** 7 - 1), ("sixteendefault", 2 ** 15 - 1),
                   ("thirtytwodefault", 2 ** 31 - 1), ("sixtyfourdefault", 2 ** 63 - 1)]:
        with self.subTest(pair=pair):
            default_val = getattr(self.int_obj.int_container, pair[0])._default
            self.assertEqual(default_val, pair[1],
  ...
  def test_set_values_outside_allowed_range(self):
    for pair in [("eightrestricted", -100), ("eightrestricted", 1001), ("sixteenrestricted", -43),
                 ("sixteenrestricted", 1001), ("thirtytworestricted", 500001), ("thirtytworestricted", -43),
                 ("sixtyfourrestricted", 72036854775809), ("sixtyfourrestricted", -43)]:
      with self.subTest(pair=pair):
        allowed = True
        try:
          ...
          allowed = False
        self.assertFalse(allowed, "Incorrectly allowed value outside of range for %s (%d)" % (pair[0], pair[1]))
    ...
    for pair in [(0, True), (10, True), (-10, False)]:
      with self.subTest(pair=pair):
        ...
        try:
          ...
          allowed = False
        self.assertEqual(allowed, pair[1], "Restricted range using max was not set correctly (%d -> %s != %s)" %
    ...
    for pair in [(0, True), (10, True), (-128, True), (-300, False)]:
      with self.subTest(pair=pair):
        ...
        try:
          ...
          allowed = False
        self.assertEqual(allowed, pair[1], "Restricted range using min was not set correctly (%s -> %s != %s)" %
          (pair[0], allowed, pair[1]))
    ...
    for pair in [(0, True), (10, True), (-128, True), (-300, False)]:
      with self.subTest(pair=pair):
        ...
        try:
          ...
          allowed = False
        self.assertEqual(allowed, pair[1], "Restricted range using min alias was not set correctly (%s -> %s != %s)" %
          (pair[0], allowed, pair[1]))
    ...
    for pair in [(0, True), (13, False), (-20, False), (5, True), (16, True)]:
      with self.subTest(pair=pair):
        ...
        try:
          ...
          allowed = False
        self.assertEqual(allowed, pair[1], "Complex range was not set correctly (%d -> %s != %s)" %
          (pair[0], allowed, pair[1]))
    ...
    for pair in [(-2, True), (42, False)]:
      with self.subTest(pair=pair):
        ...
        try:
          ...
          allowed = False
        self.assertEqual(allowed, pair[1], "Complex range with negatives was not set correctly (%d -> %s != %s)" %
          (pair[0], allowed, pair[1]))

  def test_int8_range_with_negatives_and_spaces(self):
    for pair in [(-11, False), (-5, True), (0, False), (5, False), (10, True),
                 (25, True), (31, False)]:
      with self.subTest(pair=pair):
        allowed = True
        try:
  ...
  def test_complex_range_with_equals_case(self):
    for pair in [(-43, False), (-40, True), (98, False), (122, True), (254, False),
                 (255, True), (256, False)]:
      with self.subTest(pair=pair):
        allowed = True
        try:
