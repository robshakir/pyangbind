...
def main():
    ...
    for peer in peers:
        ...
        bgp.juniper_config.bgp.peer_group[
            peer[1]].neighbor[
            peer[0]].peer_as = peer[2]
    ...
    bgp_unfilter_response = {'juniper-config': {'bgp': {'global': {'as': '2856'},
                                                        'peer-group': {'groupA': {'group-name': 'groupA',
                                                                       ...
                                                                       'groupB': {'group-name': 'groupB',
                                                                                  ...
                                                                                  'peer-type': ''}}}}}
    assert bgp.get(
    ) == bgp_unfilter_response, "unfiltered get response did not match"
    assert bgp.get(
        filter=True) == bgp_filter_response, "filtered get response did not match"
