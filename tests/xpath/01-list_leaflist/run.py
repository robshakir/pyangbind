...
def t1_leaflist(yobj, tree=False):
    ...
    for tc in [("flounder", "exists"), ("minnow", "does not exist")]:
        ...
        assert validref == True, "Reference was incorrectly set for a leaflist with" + \
            " require_instance set to false (%s threw error, but it %s)" % tc[
                1]
...
def t3_leaflist_remove(yobj, tree=False):
    ...
    for b in [("session-ipa", 1), ("amber-ale", 1), ("moose-drool", 0)]:
        ...
        assert found == b[
            1], "When retrieving a leaf-list element, a known value was not in the list (%s -> %s (%s))" % (b[0], b[1], retr[0])
        ...
        try:
            ...
            pass
        assert rem == bool(
            b[1]), "Removal of a leaflist element did not return expected result (%s -> %s != %s)" % (b[0], rem, bool(b[1]))
...
def t4_list_remove(yobj, tree=False):
    ...
    for b in [("steam", 1), ("liberty", 1), ("pygmy-owl", 0)]:
        ...
        retr = tree.get(path)
        assert len(retr) == b[
            1], "Retreive of a list element returned the wrong number of elements (%s -> %d != %d)" % (b[0], len(retr), b[1])
        rem = False
        try:
            ...
            pass
        assert rem == bool(
            b[1]), "Removal of a list element did not return expected result (%s -> %s != %s)" % (b[0], rem, bool(b[1]))
        new_retr = tree.get(path)
        assert len(
            new_retr) == 0, "An element was not correctly removed from the list (%s -> len(%s) = %d)" % (b[0], path, len(new_retr))
...
def t5_typedef_leaflist_add_del(yobj, tree=False):
    ...
    for tc in [("vancouver", True), ("burnaby", True), ("san-francisco", False), ("surrey", True), ("richmond", True)]:
        ...
        assert (tc[0] in retr[0]) == tc[
            1], "Retrieve of a leaf-list element did not return expected result (%s->%s %s)" % (tc[0], tc[1], (retr[0]))
        rem = False
        try:
            retr[0].remove(tc[0])
            rem = True
        ...
        new_retr = tree.get(path)
        assert rem == bool(
            tc[1]), "An element was not correctly removed from a leaf-list (%s -> len(%s) = %d)" % (tc[0], path, len(new_retr))
    ...
    for tc in [("gatineau", True), ("laval", True), ("new-york-city", False), ("quebec-city", True)]:
        ...
        assert expected_obj == bool(
            tc[1]), "Popped object was not the object that was expected (%s != %s)" % (tc[0], popped_obj)
        new_retr = tree.get(path)
        assert (tc[0] in new_retr[
                0]) == False, "Retrieve of a leaf-list element did not return expected result (%s->%s %s)" % (tc[0], tc[1], (new_retr[0]))
...
def t7_leaflist_of_leafrefs(yobj, tree=False):
    ...
    for b in test_list:
        ...
        assert passed == b[
            1], "A reference to a leaf-list of leafrefs was not correctly set (%s -> %s, expected %s)" % (b[0], passed, b[1])
