...
class pybindJSONEncoder(json.JSONEncoder):
  ...
  def default(self, obj, mode='default'):
    ...
    # Expand dictionaries
      ...
      return {k: self.default(v, mode=mode) for k, v in obj.items()}
    ...
    elif orig_yangt in ["int64", "uint64", "derived64"]:
...
class pybindIETFJSONEncoder(pybindJSONEncoder):
  ...
  def generate_element(obj, parent_namespace=None, flt=False,
    ...
    if generated_by == "YANGListType":
      # Handle py2/3 differences in dictionary methods
      try:
        values = obj.itervalues()
      except AttributeError:
        values = obj.values()

      return [pybindIETFJSONEncoder.generate_element(i, flt=flt,
          with_defaults=with_defaults) for i in values]
    ...
    for element_name in obj._pyangbind_elements:
      ...
      if generated_by == "container":
        ...
        # Handle py2/3 differences in dictionary methods
        try:
            values = element._members.itervalues()
        except AttributeError:
            values = element._members.values()
